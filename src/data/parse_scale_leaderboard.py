...
from src.utils.constant import SCALE_LEADERBOARD_URL, SCALE_EVAL_ADV_ROB, SCALE_COL_MODEL, LOCAL_PATH_TO_RAW_DATA, SCALE_EVAL_MAPPING, LOCAL_PATH_TO_INT_DATA
...
class ScaleLeaderbord:
    ...
    def parse_html(self) -> None:
        ...
        for table in raw_tables:
            ...
            table_pd[SCALE_COL_MODEL] = table_pd[SCALE_COL_MODEL].apply(self.remove_leading_number)
            # Rename score column
            ...
            # Split 95% CI column
            table_pd[[f"{score_new_name}_95CI_max", f"{score_new_name}_95CI_min"]] = table_pd["95% Confidence"].str.split('/', expand=True).astype(float).apply(lambda x: x + table_pd[score_new_name])
            table_pd.drop(columns="95% Confidence", axis=1, inplace=True)
            table_pd = table_pd[[SCALE_COL_MODEL, f"{score_new_name}_95CI_min", score_new_name, f"{score_new_name}_95CI_max"]]
            logger.debug(table_pd)
            tables.append(table_pd)
        # Do an outer join of all tables
        joined_table = tables.pop()
        for table in tables:
            joined_table = pd.merge(joined_table, table, on=SCALE_COL_MODEL, how="outer")
